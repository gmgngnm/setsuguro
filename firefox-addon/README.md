...
設定画面と板の見出しに `v1.9.0` のように出る。`about:addons` の「詳細」でも見られる。
...
| 待機時間 | 1秒 | 0.3／0.5／1／1.5／2／3秒。カーソルを合わせたときも、文を選んだときも同じだけ待つ |
...
左上を向くので、吹き出し自体は語や選んだ範囲の**右下**（上に出すときは右上）に
置く。指す先は語の終わり。何行にもまたがるときは最後の行の終わりを指す。角は
7pxだけ丸め、影や動きは付けない。訳した元の文は出さない（読んでいるページに
