...
設定画面と板の見出しに `v1.6.0` のように出る。`about:addons` の「詳細」でも見られる。
...
動かしている最中に出ては邪魔なので、**同じ字の上で手が止まってから1秒**で出る。
...
| カーソルを1秒あわせて意味を出す | 入 | 選ばなくても意味が出る。ツールバーの板からも入切できる |
...
見た目は掲示板くらいの素っ気なさに寄せてある。角丸も影も動きも付けず、釦や入力欄は
ブラウザの見た目のまま。読んでいるページの上に出す物なので、飾るほど邪魔になる。
