...
function goroSystemPrompt(word, morphemes, wordMeaning, avoidTexts) {
  ...
  return [
    ...
    "接辞の読みをそのまま実在しないカタカナ語（造語）として使い、「は」「が」「の」「な」などの助詞・語尾だけを付けて文中の主語・修飾語のように振る舞わせることも禁止します（例:「オクシールは」「イアリーな」のように、読みをそのまま独立した一単語として扱うのは不可）。読みの音は、必ず実在する日本語の言葉（漢字・和語や、広く定着した外来語）の一部分の音として自然に溶け込ませ、読みそのものが単独の単語のように見えないようにしてください。",
    "各接辞の読みは一音も欠かさず文中のどこかに反映してください。ただし、読みをそのまま人名・人物名のように扱うことは一切禁止します。これは「〜という名の…」「〜さん」「〜くん」といった明示的な呼び方だけでなく、「オノミが分類法を教えた」のように、読み由来の実在しないカタカナ語をそのまま文の主語にして、あたかも人物・生き物であるかのように話す・教える・歩く・笑うなどの人間的な動作をさせるパターンも含みます（「が」「は」で始まる主語であっても、それが読みそのままのカタカナ語であれば不自然な人名的用法として不可）。人物を登場させる場合は、名前ではなく役割や属性（店員、少年、隣人、先生 など）の実在する日本語の言葉で表現し、読みの音はその役割語や別の場面描写の一部分の音として組み込んでください。下品・差別的な表現も避けてください。",
...
function goroValidationPrompt(word, morphemes, candidates, wordMeaning) {
  ...
  return [
    ...
    "①読みを人名・人物名のように扱っていないこと。これは「〜という名の…」「〜さん」「〜くん」といった明示的な呼び方だけでなく、「オノミが分類法を教えた」のように、読み由来の実在しないカタカナ語をそのまま文の主語にして、あたかも人物・生き物であるかのように話す・教える・歩く・笑うなどの人間的な動作をさせているパターンも含む（「が」「は」で始まる主語であっても、それが読みそのままのカタカナ語であれば不可）。",
    ...
    "⑦接辞の読みを、そのまま実在しないカタカナ語（造語）として使い、「は」「が」「の」「な」などの助詞・語尾だけを付けて文中の主語・修飾語のように振る舞わせていないこと（例:「オクシールは」「イアリーな」のように、読みをそのまま独立した一単語として扱っているものは不可）。読みの音は、実在する日本語の言葉の一部分の音として自然に溶け込んでいる必要があります。",
