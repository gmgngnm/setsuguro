...
/* すべての接辞を2色のどちらかで囲う。
   ...
   青 ＝ 語幹に対して働く接辞。それ自体は語の中身を持たず、
        「打ち消す・逆にする・繰り返す・強める・数える」といった操作か、
        品詞を変えるだけの役目を担う。
        接頭辞なら un-（否定）・re-（再び）・over-（過度に）など、
        接尾辞なら -ation（名詞化）・-er（〜する人）など。

   オレンジ ＝ その語の意味の根幹を担う接辞。語根はもちろん、
        pre-（前）・sub-（下）・trans-（越えて）・bio-（生命）のように、
        場所・時・分野といった中身を持ち込む接頭辞もこちら。

   綴りの一覧と、意味の書かれ方の両方で見る。一覧はAIが意味を違う言い方で
   返しても取りこぼさないため、意味の側は一覧に無い接辞まで拾うため。
   in- のように語によって「中へ」とも「〜でない」とも訳される接辞は、
   その語でAIが書いた意味の側で決まる（＝語ごとに色が変わりうる） */
const FUNCTIONAL_AFFIXES = new Set([
  ...
  /* --- 接頭辞：語幹に働きかけるだけ --- */
  /* 否定・打ち消し */ "un", "non", "dis", "il", "im", "ir", "mis", "mal",
  /* 逆・対抗 */ "anti", "counter", "contra", "ob",
  /* 再び・戻す */ "re",
  /* 強調・程度 */ "be", "over", "under", "super", "hyper", "ultra", "extra",
                   "out", "per", "arch", "sur", "eu", "bene", "dys", "quasi", "pseudo",
  /* 数・量・比較 */
  "mono", "uni", "bi", "di", "tri", "tetra", "quadr", "penta", "hexa",
  "hepta", "octa", "deca", "multi", "poly", "omni", "pan", "holo",
  "semi", "hemi", "demi", "kilo", "mega", "giga", "centi", "milli",
  "deci", "nano", "equi", "iso", "homo", "hetero",
]);

/* 「人」をつくる形でも、その語自体に覚えるべき中身があるもの。
   -phile なら「好む」、-crat なら「支配」を知らないと語の意味が取れない */
const MEANINGFUL_AGENT_AFFIXES = new Set(["phile", "crat", "phobe", "naut"]);

/* 意味の書かれ方から、語幹に働きかけるだけの接辞を拾う。一覧に無い接辞や、
   語によって訳が変わる接辞（in- など）はここで決まる。
   語根を巻き込まないよう、行為者は「〜」で始まる言い回しに限っている */
const FUNCTIONAL_MEANING_RE = new RegExp([
  /* 品詞を変えるだけ */
  "(名詞|動詞|形容詞|副詞)化", "動名詞", "品詞を変え",
  /* 行為者（〜する人・〜される人・〜の専門家） */
  "[〜～][^、。]*(人|者|専門家)",
  /* 打ち消し・逆 */
  "否定", "〜でない", "打ち消", "反対", "対抗", "逆",
  ...
  "再び", "戻す", "やり直",
  /* 強調・程度 */
  "強意", "強調", "過度", "極端", "徹底", "不足", "偽の", "準〜",
  /* 誤り */
  "誤って",
  /* 数・量。「等しい・同じ・異なる」は語根 equ（等しい）まで巻き込むので
     入れない（接頭辞の equi- / iso- / homo- / hetero- は上の一覧で拾う） */
  "一つ", "二つ", "三つ", "四つ", "五つ", "六つ", "七つ", "八つ", "十の",
  "半分", "多くの", "多い・多数", "すべての", "全ての", "全体の",
  "倍", "分の一",
].join("|"));
